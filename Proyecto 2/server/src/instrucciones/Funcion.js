...
class Funcion extends instruccion_1.Instruction {
    ...
    ejecutar(env) {
        ...
        singleton_1.Singleton.tablaSimbolos.push(new TablaSimbolos_1.Simbolo(this.var_name, "Funcion", this.tipo, this.scope, this.line, this.column));
        for (const ins of this.instrucciones) {
            if (typeof ins == "object") {
                ins.ejecutar("Funcion " + this.var_name);
            }
        }
    }
    getContador() {
    ...
    getNodo(cont) {
        ...
        this.contaux = cont;
        let code = "";
        code += this.createNodoGraph(this.cont, "<Instruccion>", "Funcion");
        this.contaux++;
        //Tipo
